class Interceptor{
  ...
  go(config, isSuccess){
    for(const v of this.handlers){
      isSuccess ? v.success(config) : v.fail(config)
    }
  }
}
Interceptor.id = 1
...
class SRequest{
  ...
  request(config){
    ...
    return new Promise((resolve, reject) => {
      wx.request({
        ...
        responseType,
        success:res => {
          this.interceptRes(res, true)
          resolve(res)
        },
        fail:res => {
          this.interceptRes(res, false)
          reject(res)
        },
      })
    })
  ...
  // 遍历请求拦截器
  interceptReq(config){
    this.interceptors.request.go(config, true)
  }

  // 遍历响应拦截器
  interceptRes(response, isSuccess){
    this.interceptors.response.go(response, isSuccess)
  }
