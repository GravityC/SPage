...
const extendPage = function () {
  ...
  Page = function (name, option) {
    ...
    if (option.onNavigate) {
      const originOnNavigate = option.onNavigate;
      option.onNavigate = function (query) {
        option.$isNavigated = true
        return originOnNavigate.call(this, query)
      }
    }
    ...
    // 装饰onShow
    if (option.onShow) {
      const originOnShow = option.onShow;
...
const extendComponent = function () {
  ...
  Component = function (option) {
    ...
    // 获取当前页面实例
    option.methods.$getCurPage = function () {
      return getCurrentPages()[getCurrentPages().length - 1]
    }

    // 获取当前页面实例对应的页面名
    option.methods.$getCurPageName = function () {
      return option.methods.$getCurPage().$name
    }
